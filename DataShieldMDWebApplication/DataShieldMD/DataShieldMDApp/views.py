...
#This is for my ARX version which I can't get hierachies to work for its pretty much pointless.
'''
@login_required
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            try:
                # Apply K-Anonymity
                if form.cleaned_data.get('k_anonymity'):
                    data,identifying_fields_list = ARXWrapper.load_data(selected_file_path, identifying_fields, sensitive_fields, True)
                    data = ARXWrapper.generate_and_apply_hierarchies(data, identifying_fields_list)
                    try:
                ...
                # Apply L-Diversity
                if form.cleaned_data.get('l_diversity'):
                    data,identifying_fields_list = ARXWrapper.load_data(selected_file_path, identifying_fields, sensitive_fields, False)
                    data = ARXWrapper.generate_and_apply_hierarchies(data, identifying_fields_list)
                    try:
